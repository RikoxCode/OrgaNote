...
<template>
  <div class="flex md:flex-col lg:flex-row h-full">
    <div class="flex-1 bg-blue-500 hidden lg:flex items-center justify-center dark:bg-gray-600">
    ...
    <div class="flex-1">
      ...
      >
        <div class="text-center">
          <h1 class="text-4xl font-bold mb-6 text-gray-600 text-shadow dark:text-gray-400">Your Clubs</h1>
          <p class="mt-2 dark:text-gray-300">You are not part of any club yet.</p>
          <RouterLink
              to="/clubs/create"
      ...
      <div v-else-if="isLoading">
        <div class="flex flex-col mx-auto items-center justify-between max-w-4xl w-full p-6">
          <h1 class="text-4xl font-bold mb-6 text-gray-600 text-shadow dark:text-gray-400">Your Clubs</h1>
      ...
      <div v-else>
        <div class="flex flex-col mx-auto items-center justify-between max-w-4xl w-full p-6">
          ...
          <div class="w-full space-y-4">
            <!-- Club Cards -->
            <div class="p-4">
              <TransitionGroup name="club-fade" tag="div" appear class="space-y-4">
                <div v-for="(club, index) in clubs" :key="club.id"
                  ...
                  <div class="w-full sm:w-3/4 flex flex-col sm:flex-row items-center justify-start gap-4">
                    <div>
                      <img src="https://www.jungmusik-rottal.ch/clubdesk/w_jungmusik-rottal/fileservlet?inline=true&type=image&id=1000093&s=djEt4czuUiAA8bjlbpM9snixEtv5bhP5-CQHp991RoJdumo=&imageFormat=_512x512" alt="club.name" class="w-22 h-22 sm:w-10 sm:h-10 object-cover rounded-full"/>
                    </div>
                    <div>
                      <h2 class="text-lg md:text-xl font-semibold text-gray-800 dark:text-gray-300">{{ club.name }}</h2>
                      <div class="flex items-center gap-2 text-sm md:text-md text-gray-600 mt-1 dark:text-gray-300">
                        <Icon icon="home" class="text-gray-500"/>
                        <p>{{ club.location }}</p>
                      </div>
                    </div>
                  </div>

                  <div class="flex items-center justify-end w-full sm:w-1/4 mt-4 sm:mt-0">
                    <RouterLink :to="'/clubs/' + club.id"
                      ...
                      Visit Club
                      <Icon icon="arrow_right" class="text-xl"/>
                    </RouterLink>
                  </div>
                </div>
              </TransitionGroup>
